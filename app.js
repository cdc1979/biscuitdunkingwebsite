...
function renderSummary(summary) {
  ...
  if (statTotal) statTotal.textContent = count(summary.total_global_biscuits_dunked);
  if (statRuns) statRuns.textContent = count(summary.completed_runs);
  if (statPopular) {
    const popular = summary.most_popular_biscuit || '—';
    statPopular.textContent = popular === 'No dunked biscuits yet' ? '—' : popular;
  }
  if (statPopularRuns) statPopularRuns.textContent = summary.most_popular_biscuit_runs ? `${count(summary.most_popular_biscuit_runs)} RUNS` : 'TOP BISCUIT';
  const stats = [
  ...
  if (list) list.innerHTML = stats.map(([icon, label, value]) => `<div class="summary-row"><span class="summary-icon">${icon}</span><span class="summary-label">${label}</span><span class="summary-value">${value}</span></div>`).join('');
...
async function loadSummary() {
  if (!list && !statTotal && !statRuns && !statPopular) return;
  try {
    ...
    if (list) list.innerHTML = '<div class="score-empty">The biscuit tin is taking a tea break. Try again in a moment.</div>';
    if (updated) updated.textContent = 'GLOBAL STATS TEMPORARILY UNAVAILABLE';
    if (statTotal) statTotal.textContent = '—';
    if (statRuns) statRuns.textContent = '—';
    if (statPopular) statPopular.textContent = '—';
    if (statPopularRuns) statPopularRuns.textContent = 'TRY AGAIN LATER';
  }
}
loadSummary();
